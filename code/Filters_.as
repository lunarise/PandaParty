package code
{
	...
	public static function applyBWFilterToBitmapData(sourceBitmapData:BitmapData):BitmapData
	{
		...
		var colorToReplace:uint = 0x000000;
		var newColor:uint = 0xffffff;
		var maskToUse:uint = 0xffffff;

		var colorTwoToReplace:uint = 0x363636;

		var rect:Rectangle = new Rectangle(0,0,bwBitmapData.width,bwBitmapData.height);
		var p:Point = new Point(0,0);
		bwBitmapData.threshold(bwBitmapData,rect,p,"==",colorToReplace,newColor,maskToUse,true);
		...
		//bwBitmapData.applyFilter(bwBitmapData,bwBitmapData.rect,new Point(0,0),bw_filter);
		//apply filter to cloned bitmap data;
	...
	public static function applyContrastFilterToBitmapData(sourceBitmapData:BitmapData,severity:Number=101):BitmapData
	{
		...
		contrastBitmapData = applyBrightnessFilter(contrastBitmapData);
		
		var s:Number = severity;
		var o:Number = 168 * (1 - s); //128
		...
		contrastBitmapData.applyFilter(contrastBitmapData,contrastBitmapData.rect,new Point(0,0),contrast_filter);
		//apply filter to cloned bitmapdata;
	...
	public static function applyBrightnessFilter(sourceBitmapData:BitmapData)
	{
		...
		colorBitmapData.applyFilter(colorBitmapData,colorBitmapData.rect,new Point(0,0),brightness_filter);
		//apply filter to cloned bitmapdata;
	...
	public static function applyColorFilterToBitmapData(sourceBitmapData:BitmapData):BitmapData
	{
		...
		colorBitmapData.applyFilter(colorBitmapData,colorBitmapData.rect,new Point(0,0),color_filter);
		//apply filter to cloned bitmapdata;
